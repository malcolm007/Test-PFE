 ...
 */
Format = function(editorUi, container ,stereotype)
{
...
Format.prototype.refresh = function()
{
	...
	else
	{
		...
		label.style.width = (containsLabel) ? '50%' : '22.8%';
		var label2 = label.cloneNode(false);
		var label3 = label2.cloneNode(false);
        var label4 = label3.cloneNode(false);
        label4.style.width = (containsLabel) ? '50%' : '30.6%';
        ...
        if(graph.getSelectionCells().length === 1 && (value instanceof Object)  && (value.getAttribute('Type') !== 'Generalisation'))
        {
            // Style
            if (containsLabel)
            {
			...
			if(value.getAttribute('Type') === 'Class') {
                // Class
                mxUtils.write(label4, mxResources.get('arrange2'));
                div.appendChild(label4);
                var classPanel = div.cloneNode(false);
                classPanel.style.display = 'none';
                this.panels.push(new ClassPanel(this, ui, classPanel,this.stereotype));
                this.container.appendChild(classPanel);
                addClickHandler(label4, classPanel, idx++);
            }
            else if(value.getAttribute('Type') === 'Attribute') {
                // attribute
                mxUtils.write(label4, mxResources.get('arrange2'));
                div.appendChild(label4);
                var classPanel = div.cloneNode(false);
                classPanel.style.display = 'none';
                this.panels.push(new AttributePanel(this, ui, classPanel));
                this.container.appendChild(classPanel);
                addClickHandler(label4, classPanel, idx++);
            }
            else if(value.getAttribute('Type') === 'Method') {
                // methods
                mxUtils.write(label4, mxResources.get('arrange2'));
                div.appendChild(label4);
                var methodPanel = div.cloneNode(false);
                methodPanel.style.display = 'none';
                this.panels.push(new MethodPanel(this, ui, methodPanel,this.stereotype));
                this.container.appendChild(methodPanel);
                addClickHandler(label4, methodPanel, idx++);
            }
            else if(value.getAttribute('Type') === 'Association') {
                // attribute
                mxUtils.write(label4, mxResources.get('arrange2'));
                div.appendChild(label4);
                var associationPanel = div.cloneNode(false);
                associationPanel.style.display = 'none';
                this.panels.push(new AssociationPanel(this, ui, associationPanel));
                this.container.appendChild(associationPanel);
                addClickHandler(label4, associationPanel, idx++);
            }
        }
        else {
            label.style.width = (containsLabel) ? '50%' : '33.3%';
            label2.style.width = (containsLabel) ? '50%' : '33.3%';
            label3.style.width = (containsLabel) ? '50%' : '33.3%';
			this.currentIndex = 0;
            // Style
            if (containsLabel)
            {
...
// Arrange2
            ...
            // mxUtils.write(label4, mxResources.get('arrange2'));
            // div.appendChild(label4);
            // var attrPanel = div.cloneNode(false);
            // attrPanel.style.display = 'none';
            // this.panels.push(new AttributePanel(this, ui, attrPanel));
            // this.container.appendChild(attrPanel);
            // addClickHandler(label4, attrPanel, idx++);
		}
...
BaseFormatPanel.prototype.updateText = function(value,key,newValue)
{
    var visibility = value.substring(0,1);
    var name = value.substring(2,value.lastIndexOf(':'));
    var type = value.substring(value.lastIndexOf(':'),value.length());
...
BaseFormatPanel.prototype.createRelativeTextOption = function(label, key, width, handler, init)
{
    width = (width != null) ? width : 44;

    var graph = this.editorUi.editor.graph;
    var div = document.createElement('div');
    div.style.padding = '5px 0px 15px 1px';
    mxUtils.write(div, label);
    div.style.fontWeight = 'bold';

    function getText(value,key)
    {
            if(key === 'NAME')
            {
                return value.substring(2,value.lastIndexOf(':'));
            }
            else if(key === 'NAMEMETHOD')
            {
            ...
            else if(key === 'CLASSNAME' || key ==='NAMEASSOCIATION' || key === "NAMECARDINALITE1" || key ==='NAMECARDINALITE2')
            {
                return value;
            }

    }
    function updateText(value,key,newValue)
    {

        if(key === 'NAME')
        {
            var name = value.substring(2,value.lastIndexOf(':'));
            var visi = value.substring(0,1);
            var type = value.substring(value.lastIndexOf(':')+2,value.length);
            return visi+' '+newValue+':'+' '+type;
        }
        else if(key === 'NAMEMETHOD')
        {
            var name = value.substring(2,value.indexOf('('));
            var visi = value.substring(0,1);
            var type = value.substring(value.lastIndexOf(':')+2,value.length);
            var parameters = value.substring(value.indexOf('(')+1,value.lastIndexOf(')'));
            return visi+' '+newValue+'('+parameters+'): '+type;
        }
        else if(key === 'PARAMETERS')
        {
            var name = value.substring(2,value.indexOf('('));
            ...
            var type = value.substring(value.lastIndexOf(':')+2,value.length);
            var parameters = value.substring(value.indexOf('(')+1,value.lastIndexOf(')'));
            return visi+' '+name+'('+newValue+'): '+type;
        }
        else if(key === 'CLASSNAME' || key ==='NAMEASSOCIATION' || key ==='NAMECARDINALITE1' || key ==='NAMECARDINALITE2')
		{
			return newValue;
    ...
    function update(evt)
    {
        ...
        {
        	if(key === "NAMECARDINALITE1")
			{
                var inputValue = input.value;
                var cell = graph.getSelectionCells()[0];
                var childs = graph.getModel().getChildren(cell);
                var i =0;
                var test = true;
                var pos;
                while(i<childs.length && test)
                {
                    var tmp2 = graph.getModel().getValue(childs[i]);
                    if(tmp2.getAttribute('Type') ==='CardinaliteLabel')
                    {
                        test = false;
                        pos = i;
                    }
                ...
                var tmp = graph.getModel().getValue(childs[pos]);
                var value= tmp.cloneNode(true);

                value.setAttribute('label',updateText(value.getAttribute('label'),key,inputValue));
                graph.getModel().setValue(childs[pos],value);
			}
            else if(key === "NAMECARDINALITE2")
            {
                var inputValue = input.value;
                var cell = graph.getSelectionCells()[0];
                var childs = graph.getModel().getChildren(cell);
			...
			else if(key === "CLASSNAME" || key === "NAMEASSOCIATION" || key === "NAMEMETHOD" || key === "NAME" || key === "PARAMETERS") {
				var inputValue = input.value;
				var cell = graph.getSelectionCells()[0];
				var tmp = graph.getModel().getValue(cell);
				var value= tmp.cloneNode(true);
				value.setAttribute('label',updateText(value.getAttribute('label'),key,inputValue));
				...
				var inputValue = input.value;
				var stereo = key.substring(0,key.lastIndexOf('#'));
				var propertyName=key.substring(key.lastIndexOf('#')+1);
				
				var cell = graph.getSelectionCells()[0];
				var tmp = graph.getModel().getValue(cell);
				var value= tmp.cloneNode(true);
				var stereotype = getStereotypeByName(stereo,value);
				for(var i=0;i<stereotype.childNodes.length;i++)
				{
					if(stereotype.childNodes[i].attributes[0].name ===  propertyName)
						 stereotype.childNodes[i].attributes[0].value = inputValue;
				}
				graph.getModel().setValue(cell,value);
			}
	...
	function getStereotypeByName(stereotypeName , value){
		for(var i=0;i<value.childNodes.length;i++)
		{
			if(value.childNodes[i].getAttribute('name') === stereotypeName)
				return value.childNodes[i];
		}
	}

	function getPropertyValue(stereotype , propertyName){
		for(var i=0;i<stereotype.childNodes.length;i++)
		{
			if(stereotype.childNodes[i].attributes[0].name ===  propertyName)
				return stereotype.childNodes[i].attributes[0].value;
		}
	}

    var input = this.addTextInput(div, '', -2, width, update, 10, 5-1, handler != null);
        ...
        var listener = mxUtils.bind(this, function(sender, evt, force)
        {
            if (force || input != document.activeElement)
            {
            	if(key === "NAMECARDINALITE1"){
                    ...
                    input.value =  getText(graph.getModel().getValue(childs[pos]).getAttribute('label'),key);
				}
                else if(key === "NAMECARDINALITE2"){
                    var cell = graph.getSelectionCells()[0];
                    var tmp = graph.getModel().getValue(cell);
					...
					var cell = graph.getSelectionCells()[0];
					var tmp = graph.getModel().getValue(cell);
					input.value =  getText(tmp.getAttribute('label'),key);
				}
				else {
					var stereo = key.substring(0,key.lastIndexOf('#'));
					var propertyName=key.substring(key.lastIndexOf('#')+1);
					var cell = graph.getSelectionCells()[0];
					var tmp = graph.getModel().getValue(cell);
					var stereotype = getStereotypeByName(stereo,tmp);
					input.value = getPropertyValue(stereotype,propertyName);
				}
            }
        });
...
//panel lors d'un click sur un class

ClassPanel = function(format, editorUi, container , stereotype)
{
	BaseFormatPanel.call(this, format, editorUi, container);
	this.stereotype = stereotype;
    this.init();
};
mxUtils.extend(ClassPanel, BaseFormatPanel);
...
ClassPanel.prototype.init = function()
{
    ...
    var ss = this.format.getSelectionState();
	var ui = this.editorUi;
	var stereo = this.stereotype;
	var fieldset = document.createElement('fieldset');
    fieldset.style.margin ='20px 2px 10px 2px';
    fieldset.style.border='1px solid #C6C6C6';
    fieldset.style.borderRadius= '3px';
	
	var legend = document.createElement('legend');
    legend.style.fontWeight='bold';
    legend.style.color='#E75925';
    legend.style.marginLeft='4px';
    legend.style.padding= '0 4px';
    mxUtils.write(legend,'Class properties');
    fieldset.appendChild(legend);
	
	var div = document.createElement('div');
    div.style.padding = '5px 0px 5px 2px';
    fieldset.appendChild(this.addName(div));
    fieldset.appendChild(this.addPanel(div));
	...
	function isMandatory(stereotypes , stereoName)
	{
		for(var i=0 ; i<stereotypes.length;i++)
		{
			if(stereotypes[i].superClass === 'Mandatory' && stereotypes[i].name === stereoName)
			{
				return true;
			}
		}
	...
	for(var i=0 ; i<children.length ; i++)
	{
		...
		legend.style.marginLeft='4px';
		legend.style.padding= '0 4px';
		mxUtils.write(legend,children[i].getAttribute('name'));
		fieldset.appendChild(legend);
		var attributs = children[i].childNodes;
		for(var j=0 ; j<attributs.length ; j++)
		{
			var attr = attributs[j].attributes;
			var opacityPanel = this.createRelativeTextOption(attr[0].name+' :',children[i].getAttribute('name')+'#'+attr[0].name, 81);
			opacityPanel.style.lineHeight='25px';
			opacityPanel.style.paddingTop = '4px';
			opacityPanel.style.paddingBottom = '4px';
			fieldset.appendChild(opacityPanel);
		}
		if(isMandatory(stereo.stereotypes,children[i].getAttribute('name')) === false){
			var btn = mxUtils.button('Remove Stereotype', function(evt,index)
			{
				ui.actions.get('RemoveStereotype').funct(index);
			},i)
			
			btn.setAttribute('title', 'Remove Stereotype' + ' (' + this.editorUi.actions.get('RemoveStereotype').shortcut + ')');
    ...
    legend.style.marginLeft='4px';
    legend.style.padding= '0 4px';
    mxUtils.write(legend,'StereoTypes');
	fieldset.appendChild(legend);
	fieldset.appendChild(this.addStereotype(div));
	this.container.appendChild(fieldset);
};

ClassPanel.prototype.addPanel = function(div)
{
    var ui = this.editorUi;

    var btn = mxUtils.button(mxResources.get('attribute'), function(evt)
    {
        ui.actions.get('AddAttribute').funct();
    })

    btn.setAttribute('title', mxResources.get('attribute') + ' (' + this.editorUi.actions.get('AddAttribute').shortcut + ')');
    btn.style.width = '100px';
    btn.style.marginRight = '2px';
    div.appendChild(btn);

    var btn = mxUtils.button(mxResources.get('method'), function(evt)
    {
        ui.actions.get('AddMethod').funct();
    })

    btn.setAttribute('title', mxResources.get('method') + ' (' + this.editorUi.actions.get('AddMethod').shortcut + ')');
    btn.style.width = '100px';
    div.appendChild(btn);

    return div;
};
ClassPanel.prototype.addName = function(div)
{
    var ui = this.editorUi;
    var opacityPanel = this.createRelativeTextOption('Name :', 'CLASSNAME', 81);
    opacityPanel.style.lineHeight='25px';
    opacityPanel.style.paddingTop = '4px';
    opacityPanel.style.paddingBottom = '4px';
    //div.appendChild(opacityPanel);

    return opacityPanel;
};
ClassPanel.prototype.addStereotypeProperties = function(div)
{
    var ui = this.editorUi;
    var opacityPanel = this.createRelativeTextOption(nom, 'STEREOTYPE_PROPERTIES', 81);
    opacityPanel.style.lineHeight='25px';
    opacityPanel.style.paddingTop = '4px';
    opacityPanel.style.paddingBottom = '4px';
    //div.appendChild(opacityPanel);
...
ClassPanel.prototype.addStereotype = function(container)
{
    var ui = this.editorUi;
	var graph = ui.editor.graph;
	var stereo = this.stereotype
    ...
    mxUtils.write(div, 'Stereotype :');
    div.style.fontWeight = 'bold';
    // Adds gradient direction option
    var styleSelect = document.createElement('select');
    styleSelect.style.position = 'absolute';
    styleSelect.style.height='24px';
    styleSelect.style.border='1px solid #DDDDDD';
    styleSelect.style.color='#555555';
    styleSelect.style.marginTop = '-2px';
    styleSelect.style.right = '10px';
    styleSelect.style.width = '52%';
	
    var typesOption = document.createElement('option');
    typesOption.setAttribute('value','');
    var cell = graph.getSelectionCells()[0];
    var tmp = graph.getModel().getValue(cell);
    if(tmp.getAttribute('Stereotype') === ''){
        typesOption.setAttribute('selected', 'selected');
    }
    mxUtils.write(typesOption,'');
	styleSelect.appendChild(typesOption);
    ...
    for (var i = 0; i < this.stereotype.stereotypes.length; i++)
    {
		if(this.stereotype.stereotypes[i].superClass === 'Optional'  && getStereotypeByName(this.stereotype.stereotypes[i].name , tmp) == 0){
			var typesOption = document.createElement('option');
			typesOption.setAttribute('value',  this.stereotype.stereotypes[i].name);
			var cell = graph.getSelectionCells()[0];
			var tmp = graph.getModel().getValue(cell);
			mxUtils.write(typesOption, this.stereotype.stereotypes[i].name);
			styleSelect.appendChild(typesOption);
		}
    }
    ...
    mxEvent.addListener(styleSelect, 'change', function(evt)
    {
        ...
        try
        {
			...
			var index = 0;
			for (var i = 0; i < stereo.stereotypes.length; i++)
			{
				if(stereo.stereotypes[i].name === styleSelect.value)
					index = i;
			}
			var stereotype = stereo.stereotypes[index];
			

			var doc = mxUtils.createXmlDocument();
			var node2 = doc.createElement('stereotypes');
			
			node2.setAttribute('name',stereotype.name);
			
			for(var i=0 ; i<stereotype.attributs.length ; i++)
			{
				var node3 = doc.createElement('attributs');
				node3.setAttribute(stereotype.attributs[i].nom,'');
				node3.setAttribute('type',stereotype.attributs[i].type);
				node2.appendChild(node3);
			}
			value.appendChild(node2);
			graph.getModel().setValue(cell,value);
           ...
           /*
			...
			var value = graph.getModel().getValue(child);
			var value2= value.cloneNode(true);

			for(var i=1 ; i<cell.getChildCount();i++)
			{
				var temps = graph.getModel().getValue(cell.getChildAt(i));
				if(temps instanceof Object && temps.getAttribute('AddedBy') === 'Stereotype'){
					graph.getModel().remove(cell.getChildAt(i));
					i--;
				}
			}
			...
			{
				graph.setCellStyles(mxConstants.STYLE_STARTSIZE,26, graph.getSelectionCells());
				graph.setCellStyles('spacingBottom',5, graph.getSelectionCells());
				value2.setAttribute('label',''+styleSelect.value+'');
			}
			else {
				graph.setCellStyles(mxConstants.STYLE_STARTSIZE,40, graph.getSelectionCells());
				graph.setCellStyles('spacingBottom',0, graph.getSelectionCells());
				value2.setAttribute('label','<< '+styleSelect.value+' >>');
				var index = 0;
				for (var i = 0; i < stereo.stereotypes.length; i++)
    			{
				...
				for (var i = 0; i < stereo.stereotypes[index].attributs.length; i++)
    			{
					var doc = mxUtils.createXmlDocument();
					var node = doc.createElement('Element');
					node.setAttribute('label', '+ '+stereo.stereotypes[index].attributs[i].nom+': '+stereo.stereotypes[index].attributs[i].type);
					node.setAttribute('Type', 'Attribute');
					node.setAttribute('AddedBy', 'Stereotype');
					var field = new mxCell(node, new mxGeometry(0, 0, 100, 26), 'text;strokeColor=none;fillColor=none;align=left;verticalAlign=top;spacingLeft=4;spacingRight=4;overflow=hidden;rotatable=0;points=[[0,0.5],[1,0.5]];portConstraint=eastwest;');
					field.vertex = true;
					field.setConnectable(false);
					graph.getModel().add(cell,field,dividerIndex);
					dividerIndex++;
				}

				for (var i = 0; i < stereo.stereotypes[index].operations.length; i++)
   				{
					var node = doc.createElement('Element');
					node.setAttribute('label', '+ '+stereo.stereotypes[index].operations[i].nom+'(type): type');
					node.setAttribute('Type', 'Method');
					node.setAttribute('AddedBy', 'Stereotype');
					var method = new mxCell(node, new mxGeometry(0, 0, 140, 26), 'text;strokeColor=none;fillColor=none;align=left;verticalAlign=top;spacingLeft=4;spacingRight=4;overflow=hidden;rotatable=0;points=[[0,0.5],[1,0.5]];portConstraint=eastwest;');
					method.vertex = true;
					method.setConnectable(false);
					graph.getModel().add(cell,method,dividerIndex+1);
				}

			}
			graph.getModel().setValue(child,value2);
			*/
        }
        finally
        {
...
//Panel lors d'un click sur un attribut
...
AttributePanel.prototype.init = function()
{
    ...
    legend.style.marginLeft='4px';
    legend.style.padding= '0 4px';
    mxUtils.write(legend,'Attribute properties');
    fieldset.appendChild(legend);
	fieldset.appendChild(this.addName(fieldset));
    fieldset.appendChild(this.addType(fieldset));
    fieldset.appendChild(this.addVisibility(fieldset));

    this.container.appendChild(fieldset);

};
AttributePanel.prototype.addName = function(div)
{
    var ui = this.editorUi;

    var opacityPanel = this.createRelativeTextOption('Name :', 'NAME', 81);
    opacityPanel.style.lineHeight='25px';
    opacityPanel.style.paddingTop = '4px';
    opacityPanel.style.paddingBottom = '4px';
    //div.appendChild(opacityPanel);
...
AttributePanel.prototype.addType = function(container)
{
    var ui = this.editorUi;
    var graph = ui.editor.graph;
    var ss = this.format.getSelectionState();
    ...
    function updateText(value,key,newValue)
    {
        ...
        if(key === 'visibility')
        {
            return newValue+' '+name+':'+' '+type;
        }
        if(key === 'TYPE')
        {
            return visi+' '+name+':'+' '+newValue;
        }
    }
    var types = ['Integer', 'String', 'Boolean','Float','Class'];

    for (var i = 0; i < types.length; i++)
    {
        var typesOption = document.createElement('option');
        typesOption.setAttribute('value', types[i]);
        var cell = graph.getSelectionCells()[0];
        var tmp = graph.getModel().getValue(cell);
        if(getText(tmp.getAttribute('label'),'TYPE') === types[i]){
            typesOption.setAttribute('selected', 'selected');
        }
        mxUtils.write(typesOption, types[i]);
        styleSelect.appendChild(typesOption);
    }
    ...
    mxEvent.addListener(styleSelect, 'change', function(evt)
    {
        ...
        try
        {
            ...
            var tmp = graph.getModel().getValue(cell);
            var value= tmp.cloneNode(true);
            value.setAttribute('label',updateText(value.getAttribute('label'),'TYPE',styleSelect.value));
            graph.getModel().setValue(cell,value);
        }
        finally
        {
...
AttributePanel.prototype.addVisibility = function(container)
{
    var ui = this.editorUi;
    var graph = ui.editor.graph;
    var ss = this.format.getSelectionState();
    ...
    var types = ['+', '-', '#','~'];
    var typesLabel = ['Public', 'private', 'protected','package'];
    ...
    for (var i = 0; i < types.length; i++)
    {
        ...
        var cell = graph.getSelectionCells()[0];
        var tmp = graph.getModel().getValue(cell);
        if(getText(tmp.getAttribute('label'),'VISIBILITY') === types[i]){
            typesOption.setAttribute('selected', 'selected');
        }
        mxUtils.write(typesOption, typesLabel[i]);
        styleSelect.appendChild(typesOption);
    }
    ...
    mxEvent.addListener(styleSelect, 'change', function(evt)
    {
        ...
        try
        {
            ...
            var tmp = graph.getModel().getValue(cell);
            var value= tmp.cloneNode(true);
            value.setAttribute('label',updateText(value.getAttribute('label'),'VISIBILITY',styleSelect.value));
            graph.getModel().setValue(cell,value);
        }
        finally
        {
...
MethodPanel = function(format, editorUi, container ,  stereotype)
{
	BaseFormatPanel.call(this, format, editorUi, container);
	this.stereotype = stereotype;
    this.init();
};
mxUtils.extend(MethodPanel, BaseFormatPanel);

MethodPanel.prototype.init = function()
{
	var graph = this.editorUi.editor.graph;
	var ui = this.editorUi;
    var ss = this.format.getSelectionState();
    ...
    legend.style.marginLeft='4px';
    legend.style.padding= '0 4px';
    mxUtils.write(legend,'Method properties');
    fieldset.appendChild(legend);
	fieldset.appendChild(this.addName());
    fieldset.appendChild(this.addParameters());
    fieldset.appendChild(this.addType());
    fieldset.appendChild(this.addVisibility());
    this.container.appendChild(fieldset);
    ...
    mxUtils.write(legend,'Stereotypes');
	fieldset.appendChild(legend);
	fieldset.appendChild(this.addStereotype());

	this.container.appendChild(fieldset);
};
MethodPanel.prototype.addName = function(div)
{
    var ui = this.editorUi;

    var opacityPanel = this.createRelativeTextOption('Name :', 'NAMEMETHOD', 81);
    opacityPanel.style.lineHeight='25px';
    opacityPanel.style.paddingTop = '4px';
    opacityPanel.style.paddingBottom = '4px';
    //div.appendChild(opacityPanel);
...
MethodPanel.prototype.addParameters = function(div)
{
    var ui = this.editorUi;

    var opacityPanel = this.createRelativeTextOption('Parameters :', 'PARAMETERS', 81);
    opacityPanel.style.lineHeight='25px';
    opacityPanel.style.paddingTop = '4px';
    opacityPanel.style.paddingBottom = '4px';
    //div.appendChild(opacityPanel);
...
MethodPanel.prototype.addType = function(container)
{
    var ui = this.editorUi;
    var graph = ui.editor.graph;
    var ss = this.format.getSelectionState();
    ...
    function updateText(value,key,newValue)
    {
        ...
        {
            return visi+' '+name+'('+parameters+'):'+' '+newValue;
        }
    }
    var types = ['Integer', 'String', 'Boolean','Float','Class'];
...
MethodPanel.prototype.addVisibility = function(container)
{
    var ui = this.editorUi;
    var graph = ui.editor.graph;
    var ss = this.format.getSelectionState();
...
MethodPanel.prototype.addStereotype = function(container)
{
    var ui = this.editorUi;
	var graph = ui.editor.graph;
	var stereo = this.stereotype
    ...
    styleSelect.style.right = '10px';
    styleSelect.style.width = '52%';
	function verifier(attributes)
	{
		for(var i=0 ; i<attributes.length ; i++)
		{
			if(attributes[i].imported === 'MetaClass' && attributes[i].type === 'Operation')
				return true;
		}
		return false;
	}
	function setText(label , stereotypeLabel)
	{
		if(label.charAt(0) === '<')
		{
		...
		if(stereotypeLabel === '')
			return label;
		return ['<< ',stereotypeLabel, ' >> ',label].join('');
	}
	function getStereotypeByName(stereotypeName , value){
		for(var i=0;i<value.childNodes.length;i++)
		{
    ...
    for (var i = 0; i < this.stereotype.stereotypes.length; i++)
    {
		if(verifier(this.stereotype.stereotypes[i].attributs) && getStereotypeByName(this.stereotype.stereotypes[i].name,tmp) === 0)
		{
			var typesOption = document.createElement('option');
			typesOption.setAttribute('value',  this.stereotype.stereotypes[i].name);
			var cell = graph.getSelectionCells()[0];
    ...
    mxEvent.addListener(styleSelect, 'change', function(evt)
    {
        ...
        try
        {
            ...
            var tmp = graph.getModel().getValue(cell);
            var value= tmp.cloneNode(true);
			value.setAttribute('label',setText(value.getAttribute('label'),styleSelect.value))
...
MethodPanel.prototype.addStereotypeProperties = function(div)
{
    var ui = this.editorUi;
    var opacityPanel = this.createRelativeTextOption(nom, 'STEREOTYPE_PROPERTIES', 81);
    opacityPanel.style.lineHeight='25px';
 ...
 *
 */
AssociationPanel = function(format, editorUi, container)
{
    BaseFormatPanel.call(this, format, editorUi, container);
    this.init();
};
mxUtils.extend(AssociationPanel, BaseFormatPanel);

AssociationPanel.prototype.init = function()
{
    var graph = this.editorUi.editor.graph;
    var ss = this.format.getSelectionState();
    var fieldset = document.createElement('fieldset');
    ...
    var fieldset2 = fieldset.cloneNode(false);
    var legend2 = legend.cloneNode(false);
    mxUtils.write(legend2,'Member start');
    fieldset2.appendChild(legend2);
    var fieldset3 = fieldset.cloneNode(false);
    var legend3 = legend.cloneNode(false);
    mxUtils.write(legend3,'Member end');
    fieldset3.appendChild(legend3);

    mxUtils.write(legend,'Association properties');
    fieldset.appendChild(legend);
    fieldset.appendChild(this.addName());
    fieldset2.appendChild(this.addNameMember());
    fieldset2.appendChild(this.addCardinalite1());
    fieldset3.appendChild(this.addNameMember2());
    fieldset3.appendChild(this.addCardinalite2());
    // fieldset.appendChild(this.addParameters());
    // fieldset.appendChild(this.addType());
    // fieldset.appendChild(this.addVisibility());
    this.container.appendChild(fieldset);
    this.container.appendChild(fieldset2);
    this.container.appendChild(fieldset3);


};

AssociationPanel.prototype.addName = function(div)
{
    var ui = this.editorUi;

    var opacityPanel = this.createRelativeTextOption('Name :', 'NAMEASSOCIATION', 81);
    opacityPanel.style.lineHeight='25px';
    opacityPanel.style.paddingTop = '4px';
    opacityPanel.style.paddingBottom = '4px';
    //div.appendChild(opacityPanel);
...
AssociationPanel.prototype.addNameMember = function(div)
{
    var ui = this.editorUi;

    var opacityPanel = this.createRelativeTextOption('Name :', 'NAMECARDINALITE1', 81);
    opacityPanel.style.lineHeight='25px';
    opacityPanel.style.paddingTop = '4px';
    opacityPanel.style.paddingBottom = '4px';
    //div.appendChild(opacityPanel);
...
AssociationPanel.prototype.addNameMember2 = function(div)
{
    var ui = this.editorUi;

    var opacityPanel = this.createRelativeTextOption('Name :', 'NAMECARDINALITE2', 81);
    opacityPanel.style.lineHeight='25px';
    opacityPanel.style.paddingTop = '4px';
    opacityPanel.style.paddingBottom = '4px';
    //div.appendChild(opacityPanel);
...
AssociationPanel.prototype.addCardinalite1 = function(container)
{
    var ui = this.editorUi;
    var graph = ui.editor.graph;
    var ss = this.format.getSelectionState();
    ...
    for (var i = 0; i < types.length; i++)
    {
        ...
        var childs = graph.getModel().getChildren(cell);
        var j=0; var test = true;var pos;
        while(j< childs.length && test)
		{
			var tmp = graph.getModel().getValue(childs[j]);
			if(tmp.getAttribute('Type') === 'Cardinalite')
			{
				pos=j;
				test=false;
			}
        ...
        if(graph.getModel().getValue(childs[pos]).getAttribute('label') === types[i]){
            typesOption.setAttribute('selected', 'selected');
        }
        mxUtils.write(typesOption, types[i]);
        styleSelect.appendChild(typesOption);
    ...
    mxEvent.addListener(styleSelect, 'change', function(evt)
    {
        ...
        try
        {
            ...
            var value= graph.getModel().getValue(childs[pos]).cloneNode(true);
            value.setAttribute('label',styleSelect.value);
            graph.getModel().setValue(childs[pos],value);
        }
        finally
        {
...
AssociationPanel.prototype.addCardinalite2 = function(container)
{
    var ui = this.editorUi;
    var graph = ui.editor.graph;
    var ss = this.format.getSelectionState();
    ...
    for (var i = 0; i < types.length; i++)
    {
        ...
        var childs = graph.getModel().getChildren(cell);
        var j=childs.length-1; var test = true;var pos;
        while(j>=0 && test)
        {
            var tmp = graph.getModel().getValue(childs[j]);
            if(tmp.getAttribute('Type') === 'Cardinalite')
...
StyleFormatPanel.prototype.init = function()
{
	...
	/*this.container.appendChild(this.addClassMethods(this.createPanel()));*/
...
/*
StyleFormatPanel.prototype.addClassMethods = function(div)
{
    var ss = this.format.getSelectionState();
    if (ss.style.shape == 'swimlane') {
        ...
        var btn = mxUtils.button('Add Attribute', function (evt) {
            ui.actions.get('addAttribute').funct();
        })

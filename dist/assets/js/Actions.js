...
Actions.prototype.init = function()
{
    ...
    this.addAction('addAttribute', function()
    {
        alert(Object.keys(graph.getAllCells()));
    }, null, null, 'F2/Enter');
	...
	// Custom Actions added By me =D

    this.addAction('AddAttribute',
		function() {
				//alert('hhhh');
			graph.getModel().beginUpdate();
			try{
				...
				var test = true;
				var i =0;
				while(i<children.length && test)
				{
                    var value = graph.getModel().getValue(children[i]);
                    if(value === ''){
                        graph.getModel().add(cell,field,i);
                        test=false;
                    }
                    i++;
				}
				// value= value.cloneNode(true);
				// value.setAttribute('label','Class');
    ...
    this.addAction('AddMethod',
        function() {
            ...
            try{
				...
				graph.getModel().add(cell,field,cell.getChildCount());
		...
		this.addAction('RemoveStereotype',
        function(stereotypeIndex) {
            graph.getModel().beginUpdate();
            try{
				var cell = graph.getSelectionCells()[0];
				var tmp = graph.getModel().getValue(cell);
				var value= tmp.cloneNode(true);
				var child = value.childNodes[stereotypeIndex];
